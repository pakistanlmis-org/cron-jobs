...
$url = "http://epimis.cres.pk/API/Communication/getIssuance?date=$date&token=$token&level=province";
//echo $url;
...
AND epi_mis_kp_im_interfacing_data.transaction_type_id = 2";
...
while ($row0 = $row_select123->fetch_assoc()) {
    ...
    if (!empty($row0)) {
        ...
        $itemid = $row0['product_id'];
        $itemname = $row0['product'];
        $vvmstage = $row0['vvm_stage'];
        $purpose = $row0['purpose'];
        $transaction_reference = $row0['transaction_reference'];
...
//$tr_number = $trans['trans_no'];
            ...
            $str_qry1 = "INSERT INTO stock_master
                    ...
                    VALUES ('$transactiondate', '$transactionid', '$transaction_reference','$tr_counter','0','2','$from_warehouse_id','$to_warehouse_id','$purpose','0','1','$created_date','1','$modified_date')";
...
//echo "ManfactureId" . $manufacture_id;
...
//echo "BatchId" . $batchid;
        $qry_batch_warehouse_sel = "SELECT
                    MIN(stock_batch_warehouses.pk_id) pk_id,
        ...
        if (empty($res_select_batch_warehouse)) {
            $str_qry5 = "INSERT INTO stock_batch_warehouses
                    ...
                    VALUES ('$issuedqty', 'Running','$from_warehouse_id','$batchid','1','$created_date','1','$modified_date')";
...
//echo "BatchWhId" . $stock_batch_id;

        $conn->query("DELETE FROM stock_detail WHERE quantity='$issuedqty' AND adjustment_type = '2' AND stock_master_id='$stock_master_id' AND stock_batch_warehouse_id = '$stock_batch_id' AND created_date = '$created_date'");
        ...
        $str_qry2_detail = "INSERT INTO stock_detail
                    ...
                    VALUES ('$issuedqty', '0', '$vvmstage','1','2','$stock_master_id','$stock_batch_id','$item_unit_id','1','$created_date','1','$modified_date')";
...
mail("emailaddresshere", "IM Issued data has been updated for EPI-MIS - $date", "Inventory Mangement Issued data for EPI-MIS for the date of $date has been downloaded and processed in vLMIS. Total Downloaded vouchers: " . count($no_of_vouchers) . ", Processed vouvhers: " . count($no_of_proc_vouchers) . ", Total Downloaded Transactions: " . count($no_of_transactions) . ", Processed Transactions: " . count($no_of_proc_transactions));
